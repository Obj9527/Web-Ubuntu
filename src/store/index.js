  ...
  mutations: {
    setWindowPosition(state, payload) {
      for (let i = 0; i < state.windowList.length; i++) {
        if (state.windowList[i].id === payload.id) {
          state.windowList[i].meta.DOMRect = payload.DOMRect;
          console.log("setWindowPosition:", state.windowList[i]);
          break;
        }
      }
    },
    ...
    minimizeWindow(state, id) {
      if (state.maximizedWindowList.indexOf(id) !== -1) {
        state.maximizedWindowList.splice(
          state.maximizedWindowList.indexOf(id),
          1
        );
      }
    ...
    maximizeWindow(state, id) {
      if (state.minimizedWindowList.indexOf(id) !== -1) {
        state.minimizedWindowList.splice(
          state.minimizedWindowList.indexOf(id),
          1
        );
      }
      for (let i = 0; i < state.windowList.length; i++) {
    ...
    refreshFocusInLastWindow(state) {
      ...
      this.commit(
        ...
        state.windowList[state.windowList.length - 1].id
      );
    },
