<template>
  <Window :window="this.meta">
    <template slot="content">
      <div
        class="w-full h-full flex flex-col flex-grow overflow-y-auto select-none bg-ub-cool-grey window-main-screen"
      >
        <div
          class="md:px-40 md:py-24 md:mx-auto md:my-4 mx-auto px-14 py-10 outline-none border-0"
          style="
        ...
        ></div>
        <div
          class="flex flex-wrap justify-center items-center border-t border-gray-900"
        >
          <div
            ...
            :class="
              choosedWallpaper === wallpaper
                ? 'border-yellow-700 border-opacity-80'
                : 'border-opacity-0'
            "
            style="
              background-size: cover;
            ...
            @click.stop="changeBackgroundImage(wallpaper)"
            @mouseenter.stop="changeCursorToClick"
            @mouseleave.stop="changeCursorToDefault"
          ></div>
        </div>
      </div>
    </template>
...
export default {
  ...
  methods: {
    changeBackgroundImage(wallpaper) {
      ...
      let wpId = wallpaper.substring(
        wallpaper.indexOf("wall-"),
        wallpaper.indexOf("wall-") + 6
      );
      console.log(wpId);
