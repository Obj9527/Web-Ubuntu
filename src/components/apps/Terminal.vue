<template>
  <Window :window="this.meta">
    <template slot="content">
      <div
        class="h-full w-full bg-ub-drk-abrgn text-white text-sm font-bold overflow-y-auto window-main-screen"
        id="terminal-body"
        @click="focusInput"
      >
        <div class="flex flex-col flex-grow w-full h-full">
          <div
            :id="directive.id"
            v-for="directive in directiveStack"
            :key="directive.id"
          >
            <div class="flex w-full h-5">
              <div class="relative text-md font-bold text-ubt-green">
                obj9527@XiaoMi
              </div>
              <div class="text-white mx-px font-medium">:</div>
              <div :id="'t-path-' + directive.id" class="text-ubt-blue">
                {{ directive.currentPath }}
              </div>
              <div class="text-white mx-px font-medium mr-1">$</div>
              <div
                id="cmd"
                class="bg-transperent relative flex-1 overflow-hidden"
              >
                <span
                  :id="'t-show-' + directive.id"
                  class="float-left whitespace-pre pb-1 opacity-100 font-normal tracking-wider"
                ></span>
                <span
                  :id="'t-cursor-' + directive.id"
                  class="float-left mt-1 w-1.5 h-3.5 bg-white input-cursor"
                  v-if="
                    directive.id ===
                    directiveStack[directiveStack.length - 1].id
                  "
                ></span>
                <input
                  :id="'t-input-' + directive.id"
                  @keydown="checkKey"
                  @input="inputHandler"
            ...
            <div
              class="flex justify-start flex-wrap font-bold mr-2 text-ubt-blue"
  ...
  methods: {
      ...
      switch (key) {
        case "cd": {
          if (words.length === 0 || append === "" || append === "~") {
            ...
            this.childDirectories[this.currentDirName].includes(append)
          ) {
            this.generateNextInput();
            this.directiveStack[this.directiveStack.length - 1].currentPath +=
              "/" + append;
        ...
        default:
          result =
            "Command '" +
            directive +
            "' not found, or not yet implemented.<br>Available Commands: [ cd, ls, echo, clear, exit, mkdir ]";
      }
    ...
    focusInput() {
      let nextInput = document.getElementById(
        "t-input-" + this.directiveStack[this.directiveStack.length - 1].id
      );
      nextInput.focus();
    },
    generateUUID() {
