<template>
  <div
    class="relative bg-ub-window-title border-t-2 border-white border-opacity-5 py-1.5 px-3 text-white w-full select-none"
    :class="
      this.maximized ? 'duration-300 rounded-none' : 'rounded-lg rounded-b-none'
    "
  >
    ...
    <div
      class="absolute select-none right-0 top-0 mt-1 mr-1 flex justify-center items-center"
    >
      <WindowHeaderButton
        :button="this.buttons.minButton"
        @click.native.stop="$emit('minimize')"
        @mousedown.native.stop
      />
      <WindowHeaderButton
        :button="this.buttons.restoreButton"
        v-if="!this.isVisible"
        @click.native.stop="restoreButtonClick"
        @mousedown.native.stop
      />
      <WindowHeaderButton
        :button="this.buttons.maxButton"
        v-if="this.isVisible"
        @click.native.stop="maxButtonClick"
        @mousedown.native.stop
      />
      <WindowHeaderButton
        :button="this.buttons.closeButton"
