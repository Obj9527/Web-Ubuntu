<template>
  ...
  <div
    ...
    :class="[
      ...
      minimized ? 'opacity-0 invisible duration-500' : '',
      maximized ? 'duration-300 rounded-none' : 'rounded-lg rounded-b-none',
      focused ? 'z-20' : 'z-0',
    ]"
    :id="window.id"
    @mousedown.stop="openMenu"
  >
    <WindowHeader
      :id="window.id"
      :title="window.title"
    ...
    <WindowSidebar
      :id="window.id"
      direction="right"
      @mouseenter.native="changeCursorToColResize"
      @mouseleave.native="changeCursorToDefault"
      @mousedown.native.stop="resizeHorizontal"
    />
    <WindowSidebar
      :id="window.id"
      direction="bottom"
      @mouseenter.native="changeCursorToRowResize"
      @mouseleave.native="changeCursorToDefault"
      @mousedown.native.stop="resizeVertical"
    />
...
export default {
  ...
  computed: {
    minimized() {
      return !(
        this.$store.state.minimizedWindowList.indexOf(this.window.id) === -1
      );
    },
    maximized() {
      return !(
        this.$store.state.maximizedWindowList.indexOf(this.window.id) === -1
      );
    },
    focused() {
      return this.window.id === this.$store.state.focusedWindow;
    },
    fullHeight() {
  ...
  methods: {
    ...
    moveStart(e) {
      ...
      let mouseMoveHandler = (e) => {
        ...
        if (
          window.style.left.substring(0, window.style.left.length - 2) >= 52
        ) {
    ...
    setWindowPosition() {
      this.$store.commit("setWindowPosition", {
        id: this.window.id,
        DOMRect: this.$refs.window.getBoundingClientRect(),
      });
    },
    minimizeWindow() {
      if (
        !this.$store.state.windowList[
          this.$store.state.windowList.indexOf(this.window)
        ].meta.maximized
      ) {
        this.setWindowPosition();
      }
    ...
    restoreWindow() {
      ...
      let window = document.getElementById(this.window.id);
      window.style.width = meta.DOMRect.width + "px";
      window.style.height = meta.DOMRect.height + "px";
      window.style.top = meta.DOMRect.y + "px";
      window.style.left = meta.DOMRect.x + "px";
    ...
    resizeHorizontal(e) {
      let window = document.getElementById(this.window.id);
      let windowRect = window.getBoundingClientRect();
      let rightBorder = document.getElementById(
        "border-right-" + this.window.id
      );
    ...
    resizeVertical(e) {
      ...
      let window = document.getElementById(this.window.id);
      let windowRect = window.getBoundingClientRect();
      let bottomBorder = document.getElementById(
        "border-bottom-" + this.window.id
      );
